...
import { cloudDeleteProduct, cloudListProducts, refreshPublic } from "@/lib/adminDb";
...
export default function ProductsAdmin() {
  ...
  async function refresh() {
    try {
      setItems(await cloudListProducts());
    } catch {
      toast.error(ERR);
    } finally {
      setLoading(false);
  ...
  const filtered = useMemo(
    () => items.filter((x) => (x.name + (x.brand || "")).toLowerCase().includes(q.toLowerCase())),
    [items, q],
  );
