...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return { title: s ? s.name : "Layanan", description: s?.short_description };
}
  ...
  return (
    <div className="container-x py-10">
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-3">
        <div className="card p-5"><h2 className="font-bold">Gejala umum</h2><ul className="mt-2 list-disc pl-5 text-sm text-slate-600">{s.symptoms.map((x) => <li key={x}>{x}</li>)}</ul></div>
        <div className="card p-5"><h2 className="font-bold">Pemeriksaan</h2><ul className="mt-2 list-disc pl-5 text-sm text-slate-600">{s.diagnostics.map((x) => <li key={x}>{x}</li>)}</ul></div>
        <div className="card p-5"><h2 className="font-bold">Proses pengerjaan</h2><ol className="mt-2 list-decimal pl-5 text-sm text-slate-600">{s.process.map((x) => <li key={x}>{x}</li>)}</ol></div>
      </div>
